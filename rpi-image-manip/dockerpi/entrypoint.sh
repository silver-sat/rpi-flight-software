...
if [ "${target}" = "pi1" ]; then
  ...
  kernel="/root/qemu-rpi-kernel/kernel-qemu-5.10.63-bullseye"
  dtb="/root/qemu-rpi-kernel/versatile-pb-bullseye-5.10.63.dtb"
...
init=
if [ $stage -eq 1 ]; then
  init='systemd.run=/boot/firstrun.sh systemd.unit=kernel-command-line.target'
fi
...
exec ${emulator} \
  ...
  --dtb "${dtb}" \
  --kernel "${kernel}" \
  --append "rw earlyprintk loglevel=8 dwc_otg.lpm_enable=0 root=${root} rootwait panic=1 ${init} ${append}" \
  --no-reboot \
  --display none \
  --serial mon:stdio
